...
public class ProcessarDetalheArquivoRecebidoCIPStepServicoEJB extends StepSicoobServico {
    ...
    public RetornoExecucao executar(ContextoExecucao contexto) {
        ...
        try {
            ...
            getLogger().debug(
                    "**** Preparando Step para execução. PROCESSO: [PROCESSAR DETALHES] - REGISTROS DO ARQUIVO RECEBIDO ID [" + idArquivo + "] RANGE: [" + idLogDetRecInicial + "] A ["
                            + idLogDetRecFinal + "] ");
            ...
            return erro("Falha na gravação dos detalhes do arquivo[ID: " + idArquivo + "] ERRO: " + e.getMessage());
        }
    }
