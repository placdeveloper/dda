...
/**
 * BloquearMotorRecebimentoArquivoCIPJobServicoEJB é responsável por bloquear a instancia de processamento conforme o parametro passado para a execução do JOB
 * (PAG, BOL, BAX, RET) evitando que outros agendamentos iniciem o processamento em concorrencia.
 */
@Stateless
@Remote(JobServico.class)
public class BloquearMotorRecebimentoArquivoCIPJobServicoEJB extends MotorArquivoUtilServico {
    ...
    public VerificacaoDependencias verificarDependencias(ContextoExecucao contexto) {
        try {
            ...
            if (!ObjectUtil.isEmpty(valorParametro)) {
                return bloquearMotorProcessamentoArquivo(TipoInstanciaProcessamentoEnum.valueOf(valorParametro).getIdParametroMotorRecebimento());
            } else {
                return erro(ERRO_VERIFICAR_DEPENDENCIA_PARAMETRO);
            }
        } catch (ComumException e) {
            return erro("FALHA NO BLOQUEIO DO MOTOR DE RECEBIMENTO DE ARQUIVOS MOTIVO:[" + e.getMessage() + "]");
        }
    }
