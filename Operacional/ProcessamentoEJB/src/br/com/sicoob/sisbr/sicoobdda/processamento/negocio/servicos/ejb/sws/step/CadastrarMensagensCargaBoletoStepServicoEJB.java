...
/**
 * CadastrarMensagensCargaBoletoStepServicoEJB
 * 
 * @author Rafael.Silva
 */
@Stateless
@Remote(StepServico.class)
public class CadastrarMensagensCargaBoletoStepServicoEJB extends StepSicoobServico {
    ...
    public RetornoExecucao executar(ContextoExecucao contexto) {
        ...
        log(Constantes.STEP + Constantes.STR_SEPARACAO_2 + "CADASTRAR MENSAGENS CARGA BOLETO");
        log(Constantes.STR_SEPARACAO);
        log("************ INICIO EXECUCAO STEP CADASTRAR MENSAGENS CARGA BOLETO *************");
        log(Constantes.STR_SEPARACAO);
        ...
        try {
            ...
            log("EFETUANDO CADASTRO DE MENSAGENS CARGA BOLETO");
            getProcessarCargaMensagensDelegate().processarCadastroMensagensBoleto(numCooperativa, qtdAgrupamentoCooperativa);
            log("CADASTRO DE MENSAGENS CARGA BOLETO EFETUADO COM SUCESSO");
        } catch (ComumException e) {
            return erro(e.getMessage());
        }

        log(Constantes.STR_SEPARACAO);
        log("************ FIM EXECUCAO STEP CADASTRAR MENSAGENS CARGA BOLETO *************");
        log(Constantes.STR_SEPARACAO);
        return sucesso();
    }
    ...
    private static ISicoobLogger getLogger() {
        return SicoobLoggerPadrao.getInstance(CadastrarMensagensCargaBoletoStepServicoEJB.class);
    }
