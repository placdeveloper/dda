...
public abstract class AbstractTestUtil {
    ...
    public void testAll() throws BancoobException {
        try {
            Class<?>[] classes = getClasses("br.com.sicoob.sisbr.sicoobdda.entidadeslegado");
    ...
    private static List<Class<?>> findClasses(File directory, String packageName) throws ClassNotFoundException {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                String solucaoAlternativa = file.getName().substring(0, file.getName().length() - 6);
                if (!solucaoAlternativa.contains("EntidadesLegadoDatasource")) {
                    classes.add(Class.forName(packageName + '.' + file.getName().substring(0, file.getName().length() - 6)));
                } else {
                    System.out.println("Pulou EntidadesLegadoDatasource");
                }
            }
        }
